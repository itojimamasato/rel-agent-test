...
interface Project {
  ...
  backlogApiKey?: string;    // Backlog APIキー
...
| 認証方式 | Personal Access Token | APIキー |
| リポジトリ一覧取得 | ✅ | ✅ |
...
**認証方式**: APIキー
- ユーザーがBacklog管理画面で発行したAPIキーをプロジェクト設定で入力
- Backlogドメイン（xxx.backlog.com または xxx.backlog.jp）の指定が必要
...
2. リポジトリURLを使用してgit clone実行（HTTP認証 or SSH）
...
1. **リポジトリタイプ選択機能**
   - プロジェクト作成時にGitHub/Backlogを選択可能に

2. **Backlog連携機能**
   - Backlog MCP Serverを使用した課題取得
   - git clone経由でのソースコード取得

3. **新規API追加**
   - `/api/repo/clone`: リポジトリクローン用
   - `/api/repo/status`: クローン状態確認用
